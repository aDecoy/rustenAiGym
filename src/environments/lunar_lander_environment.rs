...
use bevy::math::Vec3;
use bevy::prelude::{
    Assets, Circle, ColorMaterial, Commands, Mesh, Mesh2d, MeshMaterial2d, Rectangle, ResMut,
    Transform, Vec2,
};
...
const GROUND_STARTING_POSITION: Vec3 = Vec3 {
    x: 0.0,
    y: -300.0,
    z: 1.0,
};

const ROOF_STARTING_POSITION: Vec3 = Vec3 {
    x: 0.0,
    y: 300.0,
    z: 1.0,
};
...
pub(crate) fn spawn_landing_target(
    mut commands: Commands,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<ColorMaterial>>,
) {
...
pub(crate) fn spawn_roof(
    ...
    commands.spawn((
        ...
        Mesh2d(meshes.add(Rectangle::default()).into()),
        MeshMaterial2d(materials.add(GROUND_COLOR)),
        Transform::from_translation(ROOF_STARTING_POSITION).with_scale(
            Vec2 {
                x: GROUND_LENGTH,
                y: 10.0,
            }
