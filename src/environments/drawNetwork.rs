...
pub fn draw_network_in_genome(mut commands: Commands,
...
pub fn draw_network_in_genome2(mut commands: Commands,
                               mut meshes: ResMut<Assets<Mesh>>,
                               mut materials: ResMut<Assets<ColorMaterial>>,
                               genome: &Genome,
) {
...
fn tegn_og_spawn_noder(commands: &mut Commands, mut meshes: ResMut<Assets<Mesh>>,
                       mut materials: ResMut<Assets<ColorMaterial>>,
...
fn tegn_forbindelser(commands: &mut Commands, mut meshes: &mut ResMut<Assets<Mesh>>, mut materials: &mut ResMut<Assets<ColorMaterial>>, genome: &Genome, point_per_node: &HashMap<Arc<NodeGene>, Vec2>) {
    for weight in genome.weight_genes.iter() {
        let firstPoint = point_per_node.get(&Arc::clone(&weight.kildenode));
        let secondPoint = point_per_node.get(&Arc::clone(&weight.destinasjonsnode));
        // nodes that never impacts outputs are not placed in a layer, and sort of irrelevant for our network
        if firstPoint.is_some() && secondPoint.is_some() {
            let points = vec![firstPoint.unwrap(), secondPoint.unwrap()];
            ...
            let vekt_color = Color::linear_rgb(
                ...
                0.1,
                if vekt_value < 0.0 { vekt_value.abs() } else { 0. }).with_alpha(1.);
...
fn lag_lag_av_nevroner_sortert_fra_output(genome: &Genome, weights_per_desination_node: &HashMap<Arc<NodeGene>, Vec<Arc<WeightGene>>>) -> (HashMap<Arc<NodeGene>, i32>, Vec<Vec<Arc<NodeGene>>>) {
    ...
    loop {
        // dbg!(&layer_index);
        let next_layer = få_neste_lag(&weights_per_desination_node, &mut node_to_layer, &mut layers_ordered_output_to_input, &mut node_to_vekt_som_flyttet_på_noden, layer_index);
        layer_index += 1;
        // dbg!(&next_layer);
        // dbg!(&next_layer.len());
